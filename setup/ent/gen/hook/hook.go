...
// Mutate calls f(ctx, m).
func (f IndividualFunc) Mutate(ctx context.Context, m gen.Mutation) (gen.Value, error) {
	mv, ok := m.(*gen.IndividualMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *gen.IndividualMutation", m)
	}
	return f(ctx, mv)
}
